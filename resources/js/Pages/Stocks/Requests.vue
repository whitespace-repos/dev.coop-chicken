        ...
        <section>
            <div class="container">
                <div class="d-flex justify-content-between align-items-center">
                    <a href="#" class="Btn mx-2 btn btn-outline-danger btn-sm" data-toggle="modal" data-target="#myModal"> New Stock Request </a>
                                ...
                                <div class="card-header p-0" id="headingOne">
                                    <ul class="d-flex justify-content-between align-items-center font-weight-bold list-unstyled mb-0">
                                        <li>
                                            ...
                                            <!--  -->
                                            <h6 class="my-2 d-inline-flex small-sm font-weight-bold ml-3 text-danger">Status - {{ request.status }}    <sub> on {{ parseDate(request.updated_at) }} </sub> <small v-if="request.status != 'Requested'" class="ml-4">Actual Payment : {{ toDecimal(request.actual_payment) }} <sup>INR</sup></small> <small v-if="request.status == 'Completed'" class="ml-3"> Receive Payment : {{ toDecimal(request.payment_received) }} <sup>INR</sup> </small></h6>
                                        </li>
                                        <li>
                                            <button class="btn btn-danger btn-sm my-1 py-0" data-toggle="modal" data-target="#receiveStockConfirmModal" @click="openReceiveConfirmationModal(request.id)" v-if="request.status == 'Sent'"><span data-feather="check-circle" class="mr-1"></span>  Receive Stock </button>
                                        </li>
                                    </ul>
                                </div>
                                                ...
                                                <form v-if="request.status == 'Approved'" @submit.prevent="savePaymentOptionForStockRequest(request.id)">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-4 mt-auto">
                                                            <button type="submit" class="btn btn-danger btn-sm px-4">Submit</button>
                                                        </div>
                                                    </div>
                                                </form>
                                            </div>
        ...
        <div class="modal CmnModal" id="receiveStockConfirmModal">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header py-3 border-danger text-danger align-items-center">
                    ...
                    <div class="modal-body">
                        <template v-if="isNotEmpty(selectedRequest)">
                            <form method="POST" @submit.prevent="ReceiveStockOnShop(selectedRequest.id)" >
                                ...
                                <button type="submit" class="btn btn-outline-danger btn-sm ml-auto"><span data-feather="check-circle" class="mr-1"></span>  Yes ,I Confirm Receive Stock</button>
                            </form>
                        </template>
                    </div>
                </div>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <form method="POST" @submit.prevent="sendStockRequest" class="mb-0">
                        <!-- Modal Header -->
                        <div class="modal-header py-2 d-flex align-items-center text-danger border-danger">
                            <h6 class="mb-0"><span data-feather="truck" class="mr-2 "></span> Request Stock</h6>
                            <button type="button" class="btn btn-sm btn-link text-danger" data-dismiss="modal">&times;</button>
                        </div>
                            ...
                            <div class="row">
                                <div class="col-4"  v-for="product in shop.products" :key="product.id">
                                    <ul class="list-group shadow-lg mb-3" v-if="product.stock">
                                        <li class="list-group-item font-weight-bold small-sm text-truncate px-2"><img  :src="product.image" class="img-fluid mr-2" width="20"/> {{ product.product_name }} <span class="text-danger">({{ toDecimal(product.association.stock) +' '+ product.weight_unit }} )</span></li>
                        ...
                        <div class="modal-footer p-0">
                            <button type="submit" class="btn btn-danger btn-sm px-5">Stock Request</button>
                        </div>
                    </form>
                </div>
            </div>
